...
# Create short, meaningful spectrum IDs from row number
optir_names <- paste0("Spec_", seq_len(nrow(optir_raw)))

optir_mat <- as.matrix(optir_raw[, -1])
wn_optir  <- as.numeric(colnames(optir_mat))

# Position along the line scan (normalised 0–1 for a colour gradient)
...
ggplot(optir_long, aes(Wavenumber, Absorbance,
  ...
  scale_x_reverse() +
  scale_colour_viridis_c(name = "Scan position", option = "C") +
